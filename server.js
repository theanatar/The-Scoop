...
function updateComment(url, request){
  ...
  if (!id || !requestComment) {
    ...
    dbComment.body = requestComment.body || dbComment.title;
...
function deleteComment(url, request){
  ...
  if(dbComment){
    var commentIds = database.users[dbComment.username].commentIds;
    commentIds.splice(commentIds.indexOf(id),1);
    var articleIds = database.articles[dbComment.articleId].commentIds;
